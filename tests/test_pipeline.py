...
class TestParseAccountIds:
    def test_single_account_with_region(self) -> None:
        targets = parse_account_ids("123456789012:us-east-1", "ap-southeast-2")
        assert targets == [AccountTarget(account_id="123456789012", region="us-east-1")]

    def test_single_account_without_region_uses_default(self) -> None:
        targets = parse_account_ids("123456789012", "ap-southeast-2")
        assert targets == [AccountTarget(account_id="123456789012", region="ap-southeast-2")]

    def test_multiple_accounts(self) -> None:
        targets = parse_account_ids(
            "123456789012:us-east-1,987654321098:ap-southeast-2", "eu-west-1"
        )
        assert targets == [
...
class TestBoolFromEnv:
    ...
    def test_truthy_strings(self, value: str) -> None:
        assert bool_from_env(value) is True

    @pytest.mark.parametrize("value", ["false", "False", "0", "no", ""])
    def test_falsy_strings(self, value: str) -> None:
        assert bool_from_env(value) is False

    def test_bool_true(self) -> None:
        assert bool_from_env(True) is True  # noqa: FBT003

    def test_bool_false(self) -> None:
        assert bool_from_env(False) is False  # noqa: FBT003
...
class TestGetRepoName:
    ...
    def test_bitbucket(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert get_repo_name() == "my-bb-repo"
...
class TestSamPipelineValidation:
    ...
    def test_single_account_resolved(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        pipe = SamPipeline()
        pipe._vars = pipe._load_and_validate()
        accounts = pipe._resolve_accounts()
        assert len(accounts) == 1
        assert accounts[0].account_id == "123456789012"
        assert accounts[0].region == "us-east-1"

    def test_multiple_accounts_resolved(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from sam_pipeline.pipe import SamPipeline

        monkeypatch.setenv(
            "AWS_ACCOUNT_IDS", "123456789012:us-east-1,987654321098:ap-southeast-2"
        )
        pipe = SamPipeline()
        pipe._vars = pipe._load_and_validate()
        accounts = pipe._resolve_accounts()
    ...
    def test_stack_name_defaults_to_repo_name(
        ...
        pipe = SamPipeline()
        pipe._vars = pipe._load_and_validate()
        assert pipe._resolve_stack_name() == "my-service"
    ...
    def test_stack_name_uses_env_var(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        pipe = SamPipeline()
        pipe._vars = pipe._load_and_validate()
        assert pipe._resolve_stack_name() == "my-custom-stack"
